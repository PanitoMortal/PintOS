...
static struct list wait_thread;
...
void remover_thread_durmiente(int64_t ticks){
	...
	struct list_elem *iter = list_begin(&wait_thread);
	while(iter != list_end(&wait_thread) ){
		struct thread *wait_thread= list_entry(iter, struct thread, elem);

		/*Si el tiempo global es mayor al tiempo que el thread permanecía dormido
		  entonces su tiempo de dormir ha expirado*/

		if(ticks >= wait_thread->timer_sleep){
